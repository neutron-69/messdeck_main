...
def menu_parse(filepath):
    ...
    excel_name = "menu.xlsx"
    excel_data = pd.read_excel(filepath,skiprows=1)
    menu_dict = {}

    excel_data.dropna(inplace=True,axis=0,how="any")
    excel_data.drop(inplace=True,labels=[10,20],axis=0)

    for columns in excel_data.columns:
        for value in excel_data[columns]:
            if "BREAKFAST" in value.upper():
                meal = "breakfast"
                menu_dict.setdefault(columns.strftime("%Y-%m-%d"),{})[meal] = []
            elif "LUNCH" in value.upper():
                meal = "lunch"
                menu_dict.setdefault(columns.strftime("%Y-%m-%d"),{})[meal] = []
            elif "DINNER" in value.upper():
                meal = "dinner"
                menu_dict.setdefault(columns.strftime("%Y-%m-%d"),{})[meal] = []
            else:
                if meal and "*" not in str(value):
    ...
    print(f"The data from {filepath} has been parsed into json file")
